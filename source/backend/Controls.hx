...
class Controls
{
	...
	var accepted:Bool = justPressed('accept'); // prevents pausing when going full screen (ALT + ENTER)
	...
	private function get_PAUSE() {
		var paused:Bool = justPressed('pause'); // prevents pausing when going full screen (ALT + ENTER)
