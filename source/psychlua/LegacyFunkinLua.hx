...
class LegacyFunkinLua {
	...
	// public static var LuaUtils.Function_Continue:Dynamic = 0;
	...
	public function new(script:String) {
		...
		set('LuaUtils.Function_Continue', LuaUtils.LuaUtils.Function_Continue);
		...
		set('framerate', ClientPrefs.data.framerateSetting);
		...
		Lua_helper.add_callback(lua, "openCustomSubstate", function(name:String, pauseGame:Bool = false) {
			...
			PlayState.instance.openSubState(new CustomSubstateLegacy(name));
		});

		Lua_helper.add_callback(lua, "closeCustomSubstate", function() {
			if(CustomSubstateLegacy.instance != null)
			{
				PlayState.instance.closeSubState();
				CustomSubstateLegacy.instance = null;
				return true;
			}
			return false;
		});
		...
		Lua_helper.add_callback(lua, "makeLuaSprite", function(tag:String, image:String, x:Float, y:Float) {
			tag = tag.replace('.', '');
			resetSpriteTag(tag);
			var leSprite:ModchartSpriteLegacy = new ModchartSpriteLegacy(x, y);
		...
		Lua_helper.add_callback(lua, "playAnim", function(obj:String, name:String, forced:Bool = false, ?reverse:Bool = false, ?startFrame:Int = 0)
		{
			if(PlayState.instance.getLuaObject(obj, false) != null) {
				...
				if(luaObj.animation.getByName(name) != null)
				{
					...
					if(Std.isOfType(luaObj, ModchartSpriteLegacy))
					{
						//convert luaObj to ModchartSpriteLegacy
						var obj:Dynamic = luaObj;
						var luaObj:ModchartSpriteLegacy = obj;
		...
		Lua_helper.add_callback(lua, "addLuaSprite", function(tag:String, front:Bool = false) {
			if(PlayState.instance.modchartSprites.exists(tag)) {
				var shit:ModchartSpriteLegacy = PlayState.instance.modchartSprites.get(tag);
				if(!shit.wasAdded) {
					if(front)
					{
		...
		Lua_helper.add_callback(lua, "removeLuaSprite", function(tag:String, destroy:Bool = true) {
			...
			var pee:ModchartSpriteLegacy = PlayState.instance.modchartSprites.get(tag);
			if(destroy) {
				pee.kill();
			}
		...
		Lua_helper.add_callback(lua, "makeLuaText", function(tag:String, text:String, width:Int, x:Float, y:Float) {
			tag = tag.replace('.', '');
			resetTextTag(tag);
			var leText:ModchartTextLegacy = new ModchartTextLegacy(x, y, text, width);
		...
		Lua_helper.add_callback(lua, "addLuaText", function(tag:String) {
			if(PlayState.instance.modchartTexts.exists(tag)) {
				var shit:ModchartTextLegacy = PlayState.instance.modchartTexts.get(tag);
				if(!shit.wasAdded) {
					getInstance().add(shit);
					shit.wasAdded = true;
					//trace('added a thing: ' + tag);
		...
		Lua_helper.add_callback(lua, "removeLuaText", function(tag:String, destroy:Bool = true) {
			...
			var pee:ModchartTextLegacy = PlayState.instance.modchartTexts.get(tag);
			if(destroy) {
				pee.kill();
			}
		...
		Lua_helper.add_callback(lua, "luaSpriteAddAnimationByPrefix", function(tag:String, name:String, prefix:String, framerate:Int = 24, loop:Bool = true) {
			...
			if(PlayState.instance.modchartSprites.exists(tag)) {
				var cock:ModchartSpriteLegacy = PlayState.instance.modchartSprites.get(tag);
				cock.animation.addByPrefix(name, prefix, framerate, loop);
				if(cock.animation.curAnim == null) {
					cock.animation.play(name, true);
				}
		...
		Lua_helper.add_callback(lua, "luaSpriteAddAnimationByIndices", function(tag:String, name:String, prefix:String, indices:String, framerate:Int = 24) {
			...
			if(PlayState.instance.modchartSprites.exists(tag)) {
				...
				var pussy:ModchartSpriteLegacy = PlayState.instance.modchartSprites.get(tag);
...
class ModchartSpriteLegacy extends FlxSprite
{
...
class ModchartTextLegacy extends FlxText
{
 	public var wasAdded:Bool = false;
...
class DebugLuaTextLegacy extends FlxText
{
 	private var disableTime:Float = 6;
 	public var parentGroup:FlxTypedGroup<DebugLuaTextLegacy>;
 	public function new(text:String, parentGroup:FlxTypedGroup<DebugLuaTextLegacy>, color:FlxColor) {
...
class CustomSubstateLegacy extends MusicBeatSubstate
{
 	public static var name:String = 'unnamed';
 	public static var instance:CustomSubstateLegacy;
 	...
 	{
 		CustomSubstateLegacy.name = name;
 		super();
...
class HScriptLegacy
{
	...
	public function new()
	{
		...
		interp.variables.set('CustomSubstate', CustomSubstateLegacy);
		#if (!flash && sys)
