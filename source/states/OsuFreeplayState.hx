...
import states.freeplay.SongBox; // When you start up song
...
class OsuFreeplayState extends MusicBeatState
{
	...
	override function create()
	{
		...
		var topBar:FlxSprite = new FlxSprite(0, -87).loadGraphic(Paths.image('freeplay/barTop'));
		topBar.setGraphicSize(1280, 152);
		topBar.screenCenter(X);
		add(topBar);

		var botBar:FlxSprite = new FlxSprite(0, 537).loadGraphic(Paths.image('freeplay/barbot'));
		botBar.setGraphicSize(1280, 119);
		botBar.screenCenter(X);
		add(botBar);
		
		var logo:FlxSprite = new FlxSprite(0, 460).loadGraphic(Paths.image('freeplay/logo'));
		logo.setGraphicSize(200, 100);
		logo.screenCenter(X);
		add(logo);
		
		fakeLogo = new FlxSprite(0, 460).loadGraphic(Paths.image('freeplay/logo'));
		...
		back = new FlxSprite(-350, 565).loadGraphic(Paths.image('freeplay/back'));
		...
		var yelSearch:FlxSprite = new FlxSprite(450, 45).loadGraphic(Paths.image('freeplay/search'));
	...
	function loadSongArray(reset:Bool, searching:Bool = false, searchQuery:String = '')
	{
		...
		for (i in 0...songs.length)
		{
			...
			songBox.loadGraphic(Paths.image('freeplay/bars/menu-button-background@2x'));
