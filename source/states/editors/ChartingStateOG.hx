	...
	override function update(elapsed:Float)
	{
		...
		if (!blockInput)
		{
			if (FlxG.keys.justPressed.ESCAPE)
			{
				...
				openSubState(new states.editors.content.EditorPlayState(getNotes(), [vocals, opponentVocals]));
			}
	...
	function getNotes():Array<Note> { 
		...
		for (secIndex in 0..._song.notes.length)
		{
			...
			for (n in sec.sectionNotes)
			{
				...
				if (daSus != null)
				{
					// convert old numeric note type to string if needed
					if (!Std.isOfType(n[3], String) && n[3] != null) n[3] = curNoteTypes[n[3]];
					note.sustainLength = daSus;
					note.noteType = n[3];
				}
				else
				{
					// event-like entry
					note.eventName = getEventName(n[1]);
					note.eventLength = n[1].length;
					if (n[1].length < 2)
					{
						note.eventVal1 = n[1][0][1];
						note.eventVal2 = n[1][0][2];
					}
					note.noteData = -1;
				}
			
				// mustPress depends on section's mustHitSection; flip if >3
				note.mustPress = sec.mustHitSection;
		...
		// Global events (if you also want them included)
		for (ev in _song.events)
		{
			var daStrumTime = ev[0];
			var note:Note = new Note(daStrumTime, 0, null, null, true);
			note.eventName = getEventName(ev[1]);
			note.eventLength = ev[1].length;
			if (ev[1].length < 2)
			{
				note.eventVal1 = ev[1][0][1];
				note.eventVal2 = ev[1][0][2];
			}
			note.noteData = -1;
			noteObjects.push(note);
		}
