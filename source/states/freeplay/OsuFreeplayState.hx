...
class OsuFreeplayState extends MusicBeatState
{
	...
	function populateSongsFromWeeks()
	{
		...
		for (i in 0...WeekData.weeksList.length)
		{
			...
			{
				var entry:Dynamic = leWeek.songs[j]; // explicit Dynamic
				var colors:Array<Int> = (entry[2] != null) ? (cast entry[2] : Array<Int>) : null;
				...
				// Ensure name and character are strings
				var name:String = Std.string(entry[0]);
				var char:String = Std.string(entry[1]);
			
				addSong(name, i, char, FlxColor.fromRGB(colors[0], colors[1], colors[2]));
			}
		}
	}
